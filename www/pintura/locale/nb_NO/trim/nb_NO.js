export default {
    ...
    statusLabelLoadImage: (state) => {
        if (!state || !state.task) return 'Venter på media';
        if (state.error)
            return state.error.code === 'IMAGE_TOO_SMALL'
                ? 'Minimum mediestørrelse er {minWidth} &times; {minHeight}'
                : 'Feil ved lasting av media';
        if (state.task === 'blob-to-bitmap') return 'Forbereder media&hellip;';
        return 'Laster media&hellip;';
    },

    statusLabelProcessImage: (state) => {
        if (!state || !state.task) return undefined;
        if (state.task === 'store') {
            if (state.error) return 'Feil ved opplasting av media';
            return 'Laster opp media&hellip;';
        }
        if (state.error) return 'Feil ved prosessering av media';
        return 'Prosessering av media&hellip;';
    },
